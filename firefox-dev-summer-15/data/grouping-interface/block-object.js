...
BlockObject.prototype.endDragHost = function(evt){
    ...
    target.removeClass("persona-dragging-host"); 
...
BlockObject.prototype.filter = function(ahost) {
    /* Regular expression provided by http://regexr.com/355hb 
       ...
       http(s)://www.example.com/path, www.example.com/path , example.com/path
       Other more subtle cases (thus rarer to be encountered) such as:
       w.yahoo. , ww.yahoo , en.yahoo, will erroneously match as a valid url.
...
BlockObject.prototype.addHost = function(ahost){
    ...
    if (!validHost) {
        //TODO: Visually notify users that they entered in an invalid URL
        //var errorMsg = '"' + ahost + '"' + ' is an invalid URL';
        //this.manager.events.trigger('invalidUrl', errorMsg);
        return; 
    }
    var parelem = this.elem();
...
BlockObject.prototype.activateTransition = function(){
    var elem = this.elem();
    var edit_persona = elem.children(".persona-edit-area");
    ...
    if(lastdom){
        var editArea_rect = edit_persona[0].getBoundingClientRect();
        ...
        var new_other_height = editArea_rect.bottom - elem_rect.top;
        if( new_height > 170 ){
            elem.css("height", new_height);
        }
        else if (editArea_rect.bottom > elem_rect.bottom) { 
            elem.css("height", new_other_height);
        }
    }
